...
public abstract class CfdpTransaction {
    ...
    protected void fault(byte conditionCode, long generatingEntityId) throws FaultDeclaredException {
        ...
        if(LOG.isLoggable(Level.SEVERE)) {
            LOG.log(Level.SEVERE, String.format("CFDP Entity [%d]: [%d] with remote entity [%d]: fault with condition code 0x%02X generated by %d detected", getLocalEntityId(), getTransactionId(), getRemoteDestination().getRemoteEntityId(), conditionCode, generatingEntityId));
        }
        // Check the handler
