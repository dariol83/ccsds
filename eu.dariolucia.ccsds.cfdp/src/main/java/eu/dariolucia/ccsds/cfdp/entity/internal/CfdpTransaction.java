...
public abstract class CfdpTransaction {
    ...
    protected void fault(byte conditionCode, long generatingEntityId) throws FaultDeclaredException {
        ...
        if(LOG.isLoggable(Level.SEVERE)) {
            LOG.log(Level.SEVERE, String.format("CFDP Entity [%d]: [%d] with remote entity [%d]: fault with condition code 0x%02X generated by entity %d detected", getLocalEntityId(), getTransactionId(), getRemoteDestination().getRemoteEntityId(), conditionCode, generatingEntityId));
        }
        // Check the handler
    ...
    protected void handlePositiveAckTimerElapsed(TimerTask timer, CfdpPdu pdu) {
        ...
        try {
            if (this.ackTimer == timer) {
                if(LOG.isLoggable(Level.INFO)) {
                    LOG.log(Level.INFO, String.format("CFDP Entity [%d]: [%d] with remote entity [%d]: positive ACK timer elapsed, current count %d", getLocalEntityId(), transactionId, getRemoteDestination().getRemoteEntityId(), this.ackTimerCount));
                }
