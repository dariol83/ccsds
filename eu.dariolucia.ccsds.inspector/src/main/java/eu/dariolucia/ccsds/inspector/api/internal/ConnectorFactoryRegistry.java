...
public final class ConnectorFactoryRegistry {
    ...
    private ConnectorFactoryRegistry() {
        this.factories = discoverAvailableConnectors();
    }

    private List<IConnectorFactory> discoverAvailableConnectors() {
        return ServiceLoader.load(IConnectorFactory.class).stream().map(ServiceLoader.Provider::get).collect(Collectors.toList());
    }
